...
NemoClaw sandbox policy for this agent. FERPA and COPPA compliant -- no student PII leaves the sandbox.
...
network:
  ...
  rules:
    ...
    - name: pd_platform
      endpoints:
        - "<PD_PLATFORM_HOST>:443"
      binaries:
        - "/usr/local/bin/claude"
      methods: ["GET"]

    - name: observation_tool
      endpoints:
        - "<OBSERVATION_HOST>:443"
      binaries:
        - "/usr/local/bin/claude"
      methods: ["GET"]
```
